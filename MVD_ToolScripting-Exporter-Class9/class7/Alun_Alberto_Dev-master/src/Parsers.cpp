...
std::vector<char> loadResourceFile(const std::string & name)
{
    ...
    // Save this into a dictionary so that we don't have to load it again
    std::vector<char> resource_file;
...
{
    // Read from the file and load the binary as a mesh
    // TO-DO
...
int Parsers::parseEntity(rapidjson::Value & entity, GraphicsSystem & graphics_system)
{
    ...
    ///TODO- Add component loading here, for each entity
